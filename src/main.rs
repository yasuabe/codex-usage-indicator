...
struct UsageSnapshot {
    ...
    primary_resets_at: Option<DateTime<Local>>,
    secondary_resets_at: Option<DateTime<Local>>,
    plan_type: Option<String>,
}
...
fn run() -> Result<(), String> {
    ...
    let snapshot = find_latest_snapshot(&input_path)?;

    println!("source: {}", snapshot.source_path.display());
    println!(
        "5h limit: {} (resets {})",
        format_percent_left(snapshot.primary_used_percent),
        format_reset(snapshot.primary_resets_at, false)
    );
    println!(
        "weekly limit: {} (resets {})",
        format_percent_left(snapshot.secondary_used_percent),
        format_reset(snapshot.secondary_resets_at, true)
    );
...
fn find_latest_snapshot(path: &Path) -> Result<UsageSnapshot, String> {
    ...
    for rollout in rollouts {
        ...
        for line in reader.lines() {
            let line = line.map_err(|err| format!("failed to read {}: {err}", rollout.display()))?;
...
fn epoch_to_local_datetime(epoch: i64) -> Option<DateTime<Local>> {
    Utc.timestamp_opt(epoch, 0).single().map(|dt| dt.with_timezone(&Local))
}

fn format_percent_left(used_percent: f64) -> String {
    format!("{:.0}% left", (100.0 - used_percent).max(0.0))
}

fn format_reset(reset_at: Option<DateTime<Local>>, with_date: bool) -> String {
    match reset_at {
